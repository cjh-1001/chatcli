...
ANALYSIS_ROLES: dict[str, dict[str, Any]] = {
    ...
    "dynamic_observer": {
        ...
        "prompt": (
            ...
            "Read all files under dynamic/ in the result directory, plus any "
            "monitor snapshots or verify snapshots if present. Extract:\n"
            "1. Execution summary: did the sample run? exit code, duration\n"
            "2. Collector events from dynamic_status.events, including start/stop order\n"
            "3. Process metrics: process_metrics.count, sample/top_memory entries, "
            "new process names/PIDs, child processes, injection or persistence clues\n"
            "4. API call trace if present: which APIs were actually called? sequence?\n"
            "5. File system changes: created/modified/deleted files\n"
            "6. Registry changes: new keys, modified values, persistence\n\n"
            ...
            "### Execution Facts\n...\n"
            "### Process Metrics\n"
            "- process_count: <n>; notable_processes: <names/PIDs/evidence>\n"
